...
// Module Rho.AudioCapture


(function ($, rho, rhoUtil) {
    'use strict';

    var moduleNS = 'Rho.AudioCapture';
    var apiReq = rhoUtil.apiReqFor(moduleNS);


    // === AudioCapture class definition ===
    ...
    // === AudioCapture instance properties ===

    rhoUtil.createPropsProxy(AudioCapture.prototype, [
    ...
    // === AudioCapture instance methods ===

    rhoUtil.createMethodsProxy(AudioCapture.prototype, [
    
          // function(/* const rho::Hashtable<rho::String, rho::String>& */ props, /* optional function */ oResult)
          { methodName: 'start', nativeName: 'start', persistentCallbackIndex: 1, valueCallbackIndex: 3 }
    ...
    rhoUtil.createRawPropsProxy(AudioCapture.prototype, [
    ]);

    // === AudioCapture constants ===

    
            AudioCapture.ENCODER_AAC = 'AAC'; 
    
            AudioCapture.ENCODER_AMR_NB = 'AMR_NB'; 
    
            AudioCapture.ENCODER_AMR_WB = 'AMR_WB'; 
    
            AudioCapture.MIC = 'mic'; 
    



    // === AudioCapture static properties ===

    rhoUtil.createPropsProxy(AudioCapture, [
    ], apiReq);

    // === AudioCapture static methods ===

    rhoUtil.createMethodsProxy(AudioCapture, [
    
    ], apiReq);

    // === AudioCapture default instance support ===
    

        rhoUtil.createPropsProxy(AudioCapture, [
            { propName: 'defaultInstance:getDefault:setDefault', propAccess: 'rw', customSet: function(obj) { if(!obj || 'function' != typeof obj.getId){ throw 'Default object should provide getId method!' }; AudioCapture.setDefaultID(obj.getId()); } }
          , { propName: 'defaultID:getDefaultID:setDefaultID', propAccess: 'rw' }
        ], apiReq);

        AudioCapture.getId = function() {
            return AudioCapture.getDefaultID();
        }

        // === AudioCapture default instance properties ===
        ...
        // === AudioCapture default instance methods ===
        ...
        // will reuse already defined methods
        rhoUtil.createRawPropsProxy(AudioCapture, [
        ]);

    

    rhoUtil.namespace(moduleNS, AudioCapture);
